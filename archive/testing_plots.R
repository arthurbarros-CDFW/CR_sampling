...
#####################################
pooling_iterations<-readRDS("outputs/basin_results_wpooling_100iterations.Rds")
target_moe=.150
CI=0.9
...
pooling_estimates<-pooling_estimates%>%
  mutate(moe_count=(upper_CI_count-lower_CI_count)/2,
         ...
         rel_count_accuracy=abs(count_bias)/true_count_natural,
...
pooling_stats<-pooling_estimates%>%
  ...
  summarize(
    ...
    mean_count_accuracy=mean(rel_count_accuracy,na.rm=TRUE),
    ...
    upper_CI_count_bias=quantile(rel_count_bias, probs = 1-(1-CI)/2, 
                                 na.rm = TRUE),
    lower_CI_count_accuracy=quantile(rel_count_accuracy, probs = (1-CI)/2, 
                                 na.rm = TRUE),
    upper_CI_count_accuracy=quantile(rel_count_accuracy, probs = 1-(1-CI)/2, 
                                 na.rm = TRUE),
    .groups = "drop"
  )

p1<-ggplot(data=pooling_stats,
           aes(x=(scales_collected),y=mean_count_moe))+
  geom_point( size=3, aes(color=factor(age)))+
  geom_hline(yintercept=target_moe,color="red")+
  ...
  scale_x_continuous(limits=c(100,5000),
                     breaks=seq(from=100,to=5000,by=200))+
  theme_bw()+
  labs(x = "basin scale samples", y = "count estimate MOE",color="Age",linetype="Age")+
  theme(strip.background = element_blank(),
        strip.text.y = element_blank(),
        legend.position = "none")+
  facet_grid(factor(age)~.)
...
p2<-ggplot(data=pooling_stats,
           aes(x=(scales_collected),y=mean_count_bias))+
  geom_point( size=3, aes(color=factor(age)))+
  geom_errorbar(data=pooling_stats,
  ...
  scale_x_continuous(limits=c(100,2000),
                     breaks=seq(from=100,to=2000,by=200))+
  labs(x = "basin scale samples", y = "count estimate bias",color="Age",linetype="Age")+
  theme(strip.background = element_blank(),
        strip.text.y = element_blank(),
        legend.position = "none")+
  facet_grid(factor(age)~.)
...
p3<-ggplot(data=pooling_stats,
           aes(x=(mean_scales_collected),y=pct_count_coverage*100,
               color = factor(age),
               linetype= factor(age)))+
  geom_point()+
  geom_line()+
  ...
  scale_y_continuous(limits=c(0,100),
                     breaks=seq(from=0,to=100,by=20))+
  labs(x = "basin scale samples", y = "count estimate coverage",color="Age",linetype="Age")+
  theme(strip.background = element_blank(),
        strip.text.y = element_blank())+
  scale_x_continuous(limits=c(100,2000),
                     breaks=seq(from=100,to=2000,by=200))+
...
p4<-ggplot(data=pooling_stats,
           aes(x=(scales_collected),y=mean_count_accuracy))+
  geom_point( size=3, aes(color=factor(age)))+
  geom_hline(yintercept=target_moe,color="red")+
  geom_errorbar(data=pooling_stats,
                aes(ymin = lower_CI_count_accuracy, 
                    ymax = upper_CI_count_accuracy,
                    color=factor(age),
                    linetype= factor(age)), 
                width = 0.2) +
  scale_x_continuous(limits=c(100,5000),
                     breaks=seq(from=100,to=5000,by=200))+
  theme_bw()+
  labs(x = "basin scale samples", y = "count estimate accuracy",color="Age",linetype="Age")+
  theme(strip.background = element_blank(),
        strip.text.y = element_blank(),
        legend.position = "none")+
  facet_grid(factor(age)~.)
...
#################################
#approach 2:with pooling count accuracy
#####################################
pooling_iterations<-readRDS("outputs/basin_results_wpooling_100iterations.Rds")
target_accuracy=.150
CI=0.95
...
pooling_estimates<-pooling_estimates%>%
  mutate(count_bias=mean_count_natural-true_count_natural,
         rel_count_bias=count_bias/true_count_natural,
         rel_count_accuracy=abs(count_bias)/true_count_natural,
         meets_target_accuracy_count=ifelse(rel_count_accuracy<=target_accuracy,T,F)
  )
...
pooling_stats<-pooling_estimates%>%
  group_by(age,scales_collected)%>%
  summarize(
    scales_collected=mean(scales_collected),
    pct_meeting_target_count_moe = mean(meets_target_accuracy_count) * 100
  )

p1<-ggplot(data=pooling_stats,
           aes(x=(scales_collected),y=pct_meeting_target_count_moe))+
  geom_point( size=3, aes(color=factor(age)))+
