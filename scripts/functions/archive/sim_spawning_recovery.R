...
#' 
#' @param k_iterations number of draws from negative-binomial distribution to 
...
sim_spawning_recovery <- function(sim_population,
                                  theta = 0.2,
                                  theta_dist = c("fixed", "beta", "uniform"),
  ...
  #handle theta distribution
  if (theta_dist[1] == "beta") {
    theta <- rbeta(1, theta_shape1, theta_shape2)
  } else if (theta_dist[1] == "uniform") {
  ...
  if (nrow(recovered_tagged) > 0) {
    ...
    unrecovered_tags <- matrix(
      stats::rnbinom(n = n_ages * k_iterations, size = N_vals, prob = theta),
      nrow = n_ages,
      ncol = k_iterations
    )
    ...
    result_dt <- data.table::data.table(
      age = rep(counts$age, k_iterations),
      N = rep(N_vals, k_iterations),
      total_tags = as.vector(total_tags),
      total_hatchery = as.vector(total_ho),
      k_iteration = rep(1:k_iterations, each = n_ages)
    )
